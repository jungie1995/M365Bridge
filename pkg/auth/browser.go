...
func (tm *TokenManager) CompleteBrowserLogin(ctx context.Context, challenge *BrowserChallenge, callback string, loginCookies, webCookies []SSOCookie) error {
	...
	if parameters.Get("error") != "" {
		return errors.New("Microsoft declined or cancelled this sign-in.")
	}
	code := parameters.Get("code")
	if code == "" {
		return errors.New("Microsoft returned no authorization code.")
	}
	tokens, err := tm.requestAuthCode(ctx, code, challenge.verifier)
	if err != nil {
		if code := aadstsCode(err.Error()); code != "" {
			return fmt.Errorf("Microsoft token exchange did not complete (%s).", code)
		}
		return errors.New("Microsoft token exchange did not complete. Try the sign-in window again.")
	}
	if tokens.RefreshToken == "" || tokens.AccessToken == "" || tokens.ExpiresIn <= 0 {
		return errors.New("Microsoft did not return renewable credentials.")
	}
