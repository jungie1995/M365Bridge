...
4. The helper first checks whether the saved Designer authorization can renew. If it can, no setup image is needed. Otherwise the same window opens Copilot and submits one small apple-image setup prompt to complete the separate Designer authorization. This prompt may appear in your Microsoft Copilot history and use a Microsoft image turn. No private project assets or prompts are sent by this check. The helper observes only the expected Designer OAuth exchange in its own tab, verifies that its account matches the text login and saves its renewal credential locally. Keep Edge open until setup finishes.
5. Both services start. Existing signed-in installations can use **start-bridges.cmd** from the install folder. **connect-microsoft.cmd** handles both first login and later re-login and starts any missing services. A text-only login is not reported as a completed image connection.

The helper requests closure of its dedicated Edge setup browser when it finishes and waits for Edge to acknowledge the request or disconnect. Cleanup has a separate three-second timeout, including after a sign-in timeout. Ordinary browser windows opened separately are outside this cleanup. Successful credential saving and browser cleanup are separate steps; an open window alone does not mean image authorization failed.
...
Text authentication and image-download authentication are different. The image path uses a separate Microsoft Designer broker token (`data/tokens/rt_broker.txt`) and Designer access-token cache. Do **not** copy either from another PC or put them in `.env`. Browser setup now completes Designer authorization explicitly. The prior HTTP-only cookie fallback can return Microsoft's JavaScript/error page even after successful text login; having fresh text cookies alone is not proof of image access. Existing credentials can renew in the background while Microsoft allows it. When that fails, use **Connect Microsoft account** on this PC to complete the real browser flow again. MFA, consent and session lifetimes cannot be bypassed or guaranteed indefinitely.

The protected readiness response reports `designer_credentials` separately: `cached`, `renewal_needed`, or `browser_sign_in_required`. These are local credential states, not image-generation QA. Image endpoints return a failure when download authorization fails; they never substitute an inaccessible signed URL for requested image bytes or relabel an HTML login page as PNG.
...
Regression tests cover first-account discovery, wrong-account rejection before credential replacement, PKCE/state checks, missing session evidence, safe save failures, and distinct image-mode readiness. Browser cleanup tests cover delayed acknowledgement, Edge disconnecting itself, and an unresponsive browser, so cleanup cannot disconnect too early or wait indefinitely. The isolated Windows installer test covers a fresh install, upgrade, paths with spaces, private-setting preservation and no unintended service startup. A real first-time account/MFA completion on a separate PC still requires the user to complete Microsoft sign-in; mocks do not prove account entitlement.
