...
4. The same window opens Copilot and submits one small apple-image setup prompt to complete the separate Designer authorization. This prompt may appear in your Microsoft Copilot history and use a Microsoft image turn. No private project assets or prompts are sent by this check. The helper observes only the expected Designer OAuth exchange in its own tab, verifies that its account matches the text login and saves its renewal credential locally. Keep Edge open until setup finishes.
